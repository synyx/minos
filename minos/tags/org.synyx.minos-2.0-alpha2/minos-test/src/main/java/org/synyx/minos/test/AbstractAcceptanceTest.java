...
/**
 * Abstract class for acceptance testing Minos sites. Setups JWebunit and
...
public abstract class AbstractAcceptanceTest extends WebTestCase {
    ...
    protected void setUp() throws Exception {
        ...
        getTestContext().setBaseUrl("http://127.0.0.1:8888" + CONTEXT_PATH);
        getTestContext().setLocale(locale);

        messageSource =
                (MessageSource) lazyLoadAppContext().getBean("messageSource");
    }


    private static ApplicationContext lazyLoadAppContext() {
    ...
    /**
     * Asserts if the static parts of a message source are contained by a given
     * message. E.g. a message source "Successfully saved user {0}!" will match
     * a message "Successfully saved user joe!", if it doesn't match the method
    ...
    protected void assertMessageSource(String code, String message) {
        ...
        for (String messageSourcePart : messageSource.trim().split(
                "\\{\\d{1,2}(,\\s?\\w+)?\\}")) {
            assertTrue(String.format(
                    "Expected text part not found in message source: [%s]\n"
                            + " Message source content was: [%s]",
                    messageSourcePart, message), message.trim().contains(
                    messageSourcePart));
        }
    }
}
