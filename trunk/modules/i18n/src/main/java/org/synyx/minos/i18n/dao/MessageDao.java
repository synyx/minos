...
public interface MessageDao extends GenericDao<Message, Long> {
     ...
     */
    @Query(
        "select m from Message m where m.basename = ?1 and m.locale.language = ?2 and m.locale.country = ?3 and m.locale.variant = ?4 order by m.key"
    )
     ...
     */
    @Query(
        "select m from Message m where m.basename = ?1 and m.locale.language = ?2 and m.locale.country = ?3 and m.locale.variant = ?4 and m.key = ?5"
    )
