...
/**
 * Initializes a {@link org.synyx.minos.core.notification.Notification} implementation based on attributes
 * specified in XML
 *
...
public class NotificationFactoryBeanDefintionParser extends AbstractSingleBeanDefinitionParser {
    ...
    private boolean isValid(Element argElement, ParserContext argParserContext) {
        ...
        if (StringUtils.hasText(message) && StringUtils.hasText(key)) {
            argParserContext.getReaderContext().error(String.format("Error creating bean: don't specify \"%1$s\" and \"%2$s\" at the same time!", ATTR_KEY, ATTR_MESSAGE), argElement);
            return false;

        } else if (!StringUtils.hasText(message) && !StringUtils.hasText(key)) {
            argParserContext.getReaderContext().error(String.format("Error creating bean: you must specify either \"%1$s\" or \"%2$s\"", ATTR_KEY, ATTR_MESSAGE), argElement);
            return false;
        }
        return true;
    }
