...
<%@ page import="org.springframework.security.web.authentication.UsernamePasswordAuthenticationProcessingFilter" %>
...
<form id="login" method="post" action="<core:url value='web/j_spring_security_check'/>">
	<table class="form">
		<tr>
			...
			<td>
				<input type="text" name="j_username" id="loginform_username" <core:if test="${not empty param.login_error}">value="<%= session.getAttribute(UsernamePasswordAuthenticationProcessingFilter.SPRING_SECURITY_LAST_USERNAME_KEY) %>"</core:if> /></td>
		</tr>
		<tr>
