...
public abstract class WebTestUtils {
    ...
    private static Message assertMessage(Model model) {
        ...
        Assert.assertTrue(String.format("No value found for message key %s!",
                Minos.MESSAGE), model.containsAttribute(Minos.MESSAGE));

        Object messageCandidate = model.asMap().get(Minos.MESSAGE);

        Assert.assertTrue(String.format(
                "Value found under %s is not an instance of %s!",
                Minos.MESSAGE, Message.class.getName()),
                messageCandidate instanceof Message);

        return (Message) model.asMap().get(Minos.MESSAGE);
    }
